...
func (r *IutReconciler) reconcile(ctx context.Context, iut *etosv1alpha2.Iut) error {
	...
	if meta.FindStatusCondition(iut.Status.Conditions, status.StatusActive) == nil {
		meta.SetStatusCondition(&iut.Status.Conditions,
			metav1.Condition{
	...
	jobSpec := &batchv1.Job{
		ObjectMeta: metav1.ObjectMeta{
			Labels: map[string]string{
				"app.kubernetes.io/name":    "iut-releaser",
					...
					Containers: []corev1.Container{
						{
							...
							Args: []string{
								...
								fmt.Sprintf("-namespace=%s", iut.GetNamespace()),
								fmt.Sprintf("-iut=%s", iut.GetName()),
							},
						},
					},
				},
