...
func main() {
	...
	flag.StringVar(&provider.environmentRequestName, "environment-request", "", "The environment request to provision for.")
...
func runProvider(provider environmentProvider) error {
	...
	if err := cli.Get(ctx, types.NamespacedName{Name: provider.environmentRequestName, Namespace: provider.namespace}, &request); err != nil {
		return err
	}
	...
	for i, iut := range iuts.Items {
		...
		if err := CreateEnvironment(ctx, &request, provider.namespace, iut, executionSpace, logArea); err != nil {
			return err
		}
		created++
	}
	if created < request.Spec.MinimumAmount {
...
func CreateEnvironment(ctx context.Context, environmentrequest *v1alpha1.EnvironmentRequest, namespace string, iut v1alpha2.Iut, executionSpace v1alpha2.ExecutionSpace, logArea v1alpha2.LogArea) error {
	cli, err := providerHelper.KubernetesClient()
	if err != nil {
		return err
	}
	...
	return cli.Create(ctx, &v1alpha1.Environment{
		...
		Spec: v1alpha1.EnvironmentSpec{
			Name:     environmentrequest.Spec.Name, // TODO: Add _SubSuite\d?
			Priority: 1,
			...
			SubSuiteID:  uuid.NewString(), // TODO: Verify
			TestRunner:  "TODO",           // TODO: Verify
			Context:     uuid.NewString(), // TODO: Verify
			...
			// Iut:         iut.Spec.ProviderData,
			Tests: environmentrequest.Spec.Splitter.Tests, // TODO: No
		},
	})
}
