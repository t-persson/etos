...
func (p *genericLogAreaProvider) Provision(ctx context.Context, logger logr.Logger, cfg provider.ProvisionConfig) error {
	environmentRequest := cfg.EnvironmentRequest
	logger.Info("Provisioning a new LogArea for EnvironmentRequest", "EnvironmentRequest", environmentRequest.Name, "Namespace", environmentRequest.Namespace)
	...
	for range cfg.MinimumAmount {
		if err := provider.CreateLogArea(ctx, environmentRequest, cfg.Namespace, v1alpha2.LogAreaSpec{
			...
			Upload: v1alpha2.Upload{
				AsJSON: false,
				Method: "GET",
				URL:    "http://etos-executionspace/executionspace/v1alpha/selftest/ping",
			},
		}); err != nil {
			return err
		}
